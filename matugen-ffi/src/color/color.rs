...
pub enum Source {
    ...
    /// Raw image bytes (base64-encoded), used for generating a color scheme
    /// without writing the image to disk first.
    ImageBytes { data_base64: String },
...
impl Source {
    pub fn is_image(&self) -> bool {
        match self {
            Source::Image { path: _ } | Source::ImageBytes { data_base64: _ } => true,
            _ => false,
        }
    }
...
pub fn get_source_color(
    ...
    let source_color: Argb = match source {
        ...
        Source::ImageBytes { data_base64 } => {
            info!("Decoding image bytes");
            let bytes = decode_base64_image(data_base64)?;
            color::get_source_color_from_image_bytes(
                &bytes,
                filter,
                fallback_color,
                &prefer,
            ...
            .wrap_err("Could not get source color from image bytes")?
        }
        #[cfg(feature = "web-image")]
        Source::WebImage { url } => {
...
/// Decodes a base64-encoded image buffer, as used by [`Source::ImageBytes`].
pub fn decode_base64_image(data_base64: &str) -> Result<Vec<u8>, Report> {
    use base64::Engine as _;
    base64::engine::general_purpose::STANDARD
        .decode(data_base64)
        .wrap_err("Could not decode base64 image data")
}

pub fn get_source_color_from_image_bytes(
    bytes: &[u8],
    filter_type: FilterType,
    fallback_color: Option<Argb>,
    prefer: &Option<SelectionPreference>,
    ...
    let ranked = get_scored_colors_from_image_bytes(bytes, filter_type, fallback_color)?;
    let ranked_formatted = format_ranked(&ranked);
...
pub fn get_scored_colors_from_image_bytes(
    bytes: &[u8],
    filter_type: FilterType,
    fallback_color: Option<Argb>,
) -> Result<Vec<Argb>, Report> {
    let mut original = ImageReader::read(bytes)?;
    let image = original.resize(112, 112, filter_type);
    let pixels: Vec<Argb> = image
        .as_pixels()
        .iter()
