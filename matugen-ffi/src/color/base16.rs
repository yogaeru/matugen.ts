...
pub fn generate_base16_schemes(source: &Source, backend: Backend) -> Result<Schemes, Report> {
    let schemes = match source {
        ...
        Source::ImageBytes { data_base64 } => {
            let bytes = crate::color::color::decode_base64_image(data_base64)?;
            let image = image::load_from_memory(&bytes)?.to_rgb8();
            generate_base16_schemes_from_image(&image, backend)
                .wrap_err("Could not generate base16 scheme from image bytes")?
        }
